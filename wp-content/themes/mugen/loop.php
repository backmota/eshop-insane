...
<?php while ( have_posts() ) : the_post(); ?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			if(!is_search()){
				...
				/* temporary not used */
				if($cf_imgurl!=""){
					$imgurl = '<div class="frameimg"><div class="curve-down"><a href="'.get_permalink().'"><img src='. $cf_imgurl .' alt="'. get_the_title( $post->ID ).'" class="scale-with-grid"/></a></div></div>';
				}elseif(has_post_thumbnail($post->ID) ){
					$imgurl = '<div class="frameimg"><div class="curve-down"><a href="'.get_permalink().'">'.get_the_post_thumbnail($post->ID, 'blog-post-image', array('class' => 'scale-with-grid')).'</a></div></div>';
				}else{
					$imgurl ="";
				}
				echo '<div class="framecontainer">';
					echo $imgurl;
